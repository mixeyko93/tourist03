...
Nearby-координаты существуют только в памяти страницы и параметрах текущего
API-запроса. Они не записываются в PostgreSQL, cookie, local history,
application analytics или server event payload. Пользователь может отказать в
геолокации и выбрать город/точку вручную.

Недавние запросы (не более 10) и просмотренные страницы (общий предел 20)
хранятся только в `localStorage`, не синхронизируются и очищаются одной
кнопкой. Повреждённые записи игнорируются.

`content.discovery_daily_metrics` хранит только дневные агрегаты allowlisted
событий, тип/slug опубликованного контента или редакционный topic key. Raw
query, точные координаты, user/account id, IP и User-Agent отсутствуют в
схеме. Retention — 400 дней; более старые агрегаты удаляются при записи новой
метрики. Popular не строится из редких сырых запросов.
