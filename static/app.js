...
function openBookingConfirmationModal({ camp, campId, rooms, filter, onBack, initialItems }) {
  ...
  // Логика кнопки автоподбора апартаментов
    ...
    // Показываем кнопку только если есть недораспределённые гости
	    if (allocatedGuests < totalGuests && availableRooms.length > 0) {
	      ...
	      if (autoPickVariants.length === 0) {
	        ...
	        autoPickBtn.textContent = `Подбор ${housingLabel} для вас`;
	        autoPickBtn.disabled = autoPickActive;
	        autoPickBtn.style.opacity = autoPickActive ? '0.5' : '1';
	      } else {
...
// ===== ПРИВЕТСТВИЕ И НОВЫЕ КНОПКИ НА КАРТЕ =====
function showWelcomeOnce() {
  const WELCOME_SHOWN = 'welcome_shown_v1';
  if (localStorage.getItem(WELCOME_SHOWN)) return; // Уже показали один раз
  ...
  const moreBtn = document.getElementById('welcomeMore');
...
// Инициализируем приветствие при загрузке
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => showWelcomeOnce(), 500);
}, { once: true });
  ...
  faqBtn.onclick = (e) => {
    ...
    const faqContent = `
      <div class="modal-card" style="max-width:500px">
        <div style="font-size:20px;font-weight:700;margin-bottom:16px">❓ Часто задаваемые вопросы</div>
        
        <div class="faq-section">
          <div class="faq-q">🗺️ Как искать кемпинги на карте?</div>
          <div class="faq-a">Пролистайте карту, нажимайте на иконки лагерей. В попапе можно открыть подробнее или забронировать напрямую.</div>
        </div>
        
        <div class="faq-section">
          <div class="faq-q">📅 Как забронировать место?</div>
          <div class="faq-a">Выберите кемпинг → нажмите «Забронировать» → выберите даты и количество гостей → выберите номер/дом → подтвердите бронь.</div>
        </div>
        
        <div class="faq-section">
          <div class="faq-q">👤 Как войти в личный кабинет?</div>
          <div class="faq-a">Откройте вкладку «Личный кабинет» → введите номер телефона → подтвердите код из SMS. Для первой регистрации выберите «Регистрация».</div>
        </div>
        
        <div class="faq-section">
          <div class="faq-q">💰 Как оплачивается бронирование?</div>
          <div class="faq-a">После подтверждения брони администратор отправит вам реквизиты для оплаты. Тестовый код подтверждения: 0000</div>
        </div>
        
        <div class="faq-section">
          <div class="faq-q">🏕️ Как узнать подробнее о кемпинге?</div>
          <div class="faq-a">Нажмите на иконку лагеря на карте → кнопка «Подробнее» → всю информацию, фото и параметры номеров.</div>
        </div>
        
        <div class="faq-section">
          <div class="faq-q">❌ Как отменить бронирование?</div>
          <div class="faq-a">Откройте личный кабинет → найдите бронь → кнопка «Отменить». Условия возврата зависят от политики кемпинга.</div>
        </div>
        
        <div style="margin-top:18px;padding-top:18px;border-top:1px solid var(--border-color)">
          <div style="font-size:13px;color:var(--tg-hint);margin-bottom:12px">📞 Не нашли ответ? Свяжитесь с поддержкой:</div>
          <button class="button primary" id="contactSupport" style="width:100%">Написать в поддержку</button>
        </div>
        
        <button class="button ghost" id="faqClose" style="width:100%;margin-top:10px">Закрыть</button>
      </div>
    `;
    
    showModal(faqContent);
    ...
    if (supportBtn) {
      supportBtn.onclick = () => {
        ...
        if (isTG && window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.openLink) {
          Telegram.WebApp.openLink('https://t.me/tourist_03_bot');
        } else {
          alert('Напишите в Telegram: @tourist_03_bot');
        }
      };
    }
  };
...
function initShareButton() {
  ...
  shareBtn.onclick = (e) => {
    ...
    if (isTG && window.Telegram && window.Telegram.WebApp && window.Telegram.WebApp.shareToChat) {
      Telegram.WebApp.shareToChat(
        {
          url: window.location.href,
          text: 'Привет! 👋\n\nНашел отличное приложение для бронирования кемпингов - Турист_03. Смотри и бронируй прямо здесь! ⛺️🏕️\n\nСкорее переходи!'
        },
        () => {
          showSnackbar({ message: '✅ Поделено!' });
        }
      );
    } else if (navigator.share) {
      navigator.share({
        title: 'Турист_03 ⛺️',
        text: 'Найди идеальный кемпинг для отдыха! 🏕️',
        url: window.location.href,
      }).catch(() => {
        showSnackbar({ message: 'Не удалось поделиться' });
      });
    } else {
      ...
      sheet.innerHTML = `
        <div class="modal-card" style="text-align:center;max-width:400px">
          <div style="font-size:18px;font-weight:700;margin-bottom:12px">Поделиться приложением</div>
          <input id="shareLink" type="text" value="${window.location.href}" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:8px;background:rgba(255,255,255,0.03);color:var(--tg-text);font-size:12px;margin-bottom:12px" readonly>
          <div style="display:flex;gap:10px">
            <button class="button primary" id="copyLink" style="flex:1">Копировать</button>
            <button class="button ghost" id="closeShare" style="flex:1">Отмена</button>
          </div>
        </div>
      `;
      document.body.appendChild(sheet);
      
      const copyBtn = sheet.querySelector('#copyLink');
      const closeBtn = sheet.querySelector('#closeShare');
      const input = sheet.querySelector('#shareLink');
      ...
      if (copyBtn) {
        copyBtn.onclick = () => {
          try {
            ...
            showSnackbar({ message: '❌ Не удалось скопировать' });
          }
        };
      }
      
      if (closeBtn) closeBtn.onclick = () => sheet.remove();
      sheet.addEventListener('click', (e) => {
        if (e.target === sheet) sheet.remove();
      });
    }
